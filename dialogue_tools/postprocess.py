...
def load_banks(bank_specs):
    """[[ch, dir], ...] を {ch: [clip[T], ...]} に読む(相槌=聞き手の声)。"""
    banks = {}
    for ch, d in bank_specs or []:
        ch = int(ch)
        clips = [torchaudio.load(str(p))[0].reshape(-1)
                 for p in sorted(Path(d).glob("*.wav"))]
        if clips:
            banks[ch] = clips
        else:
            print(f"[backchannel] ch{ch}: {d} に *.wav が無い。", flush=True)
    return banks


def inject_backchannels(turns, audios, onsets_samp, banks, min_turn_s=3.0,
                        rate=0.6, seed=0, gain=0.6):
    """相手の長ターン中に、聞き手chへ相槌素片を挿入する。

    banks = {channel: [clip[T], ...]} で、話者ch の相手(聞き手)ch の声バンクから
    借りる。onsets_samp は最終タイムライン上の各ターン開始位置(overlap後でも整合)。
    ...
    for t, a, onset in zip(turns, audios, onsets_samp):
        ...
        if t["duration"] >= min_turn_s and listener_ch in banks and rng.random() < rate:
            clip = rng.choice(banks[listener_ch]) * gain
            pos = onset + int(n * rng.uniform(0.4, 0.8))  # ターンの40〜80%位置
            inserts.append({"channel": listener_ch, "onset_samp": pos, "audio": clip})
    print(f"[backchannel] {len(inserts)} 個挿入(聞き手の声)。", flush=True)
    return inserts
...
def main():
    ...
    ap.add_argument("--overlap_ms", type=float, default=250.0)
    ...
    ap.add_argument("--bc_min_turn_s", type=float, default=3.0)
    ap.add_argument("--bc_rate", type=float, default=0.6)
    ...
    if "backchannel" in args.op:
        ...
        inserts = inject_backchannels(turns, audios, onsets, banks,
                                      args.bc_min_turn_s, args.bc_rate, args.seed,
                                      args.bc_gain)
        if inserts:
