...
class FireRedTTS2:
    ...
    def generate_dialogue(
        ...
        output_stereo=False (default): mono [1, T], all turns concatenated.
        output_stereo=True: turn-separated stereo [2, T] with S1/S3 -> left,
            S2/S4 -> right (no overlap). The per-turn audio is identical to the
            mono path; only the final channel assembly differs. Useful for
            speaker-separated corpora (e.g. Moshi-style 2ch training data).
        """
        ...
        if output_stereo:
            # Turn-separated stereo: place each turn on its speaker's channel
            # (S1/S3 -> left, S2/S4 -> right), no overlap. Same per-turn audio.
            device = all_storage_segments[0].audio.device
            total = sum(seg.audio.shape[1] for seg in all_storage_segments)
            all_audio = torch.zeros(2, total, device=device)
            off = 0
            for seg in all_storage_segments:
                n = seg.audio.shape[1]
                ch = 0 if seg.speaker in ("[S1]", "[S3]") else 1
                all_audio[ch, off : off + n] = seg.audio.reshape(-1)
                off += n
            all_audio = all_audio.cpu()
            return all_audio

        # Concatenate all generations (mono)
